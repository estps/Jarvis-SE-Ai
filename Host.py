...
local function fetchTunnelUrl()
    local ok, resp = pcall(http.get, RAW_URL, {}, false, false, 10000)
    if ok and resp then
...
local function drawText(px, py, s, fg, bg)
    ...
    for r = 1, 7 do
        ...
        for i = 1, #s do
            ...
            local line = g[r] or G["?"][r]
            for c = 1, 5 do
                row = row .. (line:sub(c, c) == "#" and fg .. fg or bg .. bg)
            end
            row = row .. bg .. bg
        end
        rows[r] = row
    ...
    pcall(term.drawPixels, px, py, rows2)
end

local FONT = 2
local CHAR_W = FONT * 6
local LINE_H = FONT * 8
local CPL = math.floor((gw - 12) / CHAR_W)
local VIS = 16

local function drawTextR(px, py, s, fg, bg)
    drawText(px - #s * CHAR_W + 3, py, s, fg, bg)
end
...
local function visLines()
    local v = math.floor((gh - 40) / LINE_H)
...
local function render()
    term.setFrozen(true)
    rect(0, 0, gw, gh, P.bg)
    rect(0, 0, gw, 19, P.head)
    rect(0, 19, gw, 1, P.bd)
    drawText(6, 4, "JARVIS", char(P.accent), char(P.head))
    local stTxt = "OFFLINE"
    local stCol = P.err
    if online then stTxt = "ONLINE" end
    if online then stCol = P.ok end
    local stW = #stTxt * CHAR_W + 8
    rect(gw - 6 - stW, 2, stW, 16, P.bd)
    drawTextR(gw - 10, 4, stTxt, char(stCol), char(P.bd))
    drawTextR(gw - 6 - stW - 4, 4, "NODES " .. tostring(nodeCount), char(P.dim), char(P.head))
    local iy = gh - 20
    rect(0, iy, gw, 20, P.input)
    rect(0, iy, gw, 1, P.inbd)
    rect(0, iy + 19, gw, 1, P.inbd)
    local msgTop = 20
    if dbgOn then
        drawText(6, 20, lastEv .. " " .. evP1 .. "|" .. bytesStr(input), char(P.warn), char(P.bg))
        msgTop = 36
    end
    local n = #buf
    ...
    local lineNo = 0
    for i = from, to do
        drawText(6, msgTop + lineNo * LINE_H, buf[i].txt, char(buf[i].fg), char(P.bg))
        lineNo = lineNo + 1
    end
    drawText(6, iy + 3, ">", char(P.dim), char(P.input))
    drawText(12, iy + 3, input, cW, char(P.input))
    if math.floor(os.epoch("utc") / 500) % 2 == 0 then
        rect(12 + #input * CHAR_W, iy + 3, 4, 12, P.cur)
    end
...
local function mainGraphics()
    ...
    CPL = math.floor((gw - 14) / CHAR_W)
    VIS = visLines()
    dbg("gfx", gw, gh, "cpl", CPL, "vis", VIS)
    fetchTunnelUrl()
    sysLine("Tunnel: " .. TUNNEL_URL)
    testConnection()
    fetchNodes()
